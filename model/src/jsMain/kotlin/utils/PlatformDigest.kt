...
actual class PlatformDigest {
    private val md5MessageDigest = MD5MessageDigest()

    actual fun md5(data: ByteArray): ByteArray = md5MessageDigest.compute(data)
}

actual val Digest: PlatformDigest = PlatformDigest()


private class MD5MessageDigest(
    ...
    fun compute(message: ByteArray): ByteArray {
        ...
        for (i in 0 until numBlocks) {
            ...
            for (j in 0..63) {
                ...
                val temp = b + a + f + buffer[bufferIndex] +
                        tableT[j].rotateLeft(
                            shift[(div16 shl 2) or (j and 3)]
                        )
                a = d
                d = c
                c = b
